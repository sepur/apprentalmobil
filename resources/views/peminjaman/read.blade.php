...
<table  id="example1" class="table table-bordered table-striped">
  <thead>
    <tr class="judul">
      ...
      <th scope="col">No Telpon Pengguna</th>
  ...
  <tbody>    
    ...
    <tr>
      ...
      <td><button class="btn btn-sm btn-primary" value="pengambalian"  onClick="pengembalian({{$item->id}})">{{$item->get_pelanggan->nama}}</td>   
      <td>{{$item->get_pelanggan->alamat}}</td>  
      <td>{{$item->get_pelanggan->no_telp}}</td>  
      <!-- <td>{{$item->get_mobil->get_jenis->nama}}</td>   
      <td>{{$item->get_mobil->get_merk->nama}}</td>   -->
      ...
      <td class="btn-action">
        <div>
            <img src="{{ asset('storage/images/mobil/' . $item->get_mobil->gambar) }}" style="width:80px;height:80px;"></img>
        </div>    
      </td>                                                                      
